...
注意，因为是动态维护，因此迭代器指向可能会被修改  

迭代器存储容器指针，第一层list指针，第二层list指针，其中第二层list指针不会被修改，因此可以用于判断是否相等  

但是如果进行了动态维护可能导致第一层list指针被修改，自增、自减、push_back、push_front、顺序insert均不会进行修改。  

当涉及到迭代器 +、- 时或者使用maintain时可能进行维护导致修改，因此除进行次操作的迭代器外的迭代器需要重新赋值。  
